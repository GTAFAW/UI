...
-- ===================== Acrylic UI 主框架（整合传送功能，Tab改为Tabs.xgo3） =====================
local AcrylicLibrary, Window, Tabs, Notification, NotificationHolder
local Players = game:GetService("Players")
local LocalPlayer = Players.LocalPlayer
local PlayerButtons = {}   -- 存储玩家按钮与监听连接
local PlayerDropdown       -- 下拉控件引用
local selectedName = ""    -- 选中的玩家名
...
if not success then
    ...
    -- 简易消息提示（兼容无通知库场景）
    local function tempMsg(title, desc)
    ...
    tempMsg("UI加载失败", "无法加载主界面，脚本功能不可用")
    return -- 核心UI失效，终止脚本
end
...
local function GetPlayerNames()
    local t = {}
    for _, p in ipairs(GetOtherPlayers()) do table.insert(t, p.Name) end
...
-- 3. 传送UI组件生成函数
-- 创建单个玩家传送按钮
local function CreateSinglePlayerButton(player, parentTab)
    -- 清理已存在的按钮
...
local function RefreshDropdown()
    ...
    local realOnlineNames = GetPlayerNames()
    PlayerDropdown.Values = realOnlineNames
    PlayerDropdown.Default = realOnlineNames[1]
    if PlayerDropdown.Update then PlayerDropdown:Update() end
    -- 清理失效选中
    if selectedName ~= "" and not table.find(realOnlineNames, selectedName) then
        selectedName = ""
        SendMsg("选中失效", "之前选中的玩家已离线，已清空选中状态", "default")
    end
end
...
-- 6. 填充Tabs.xgo3标签页内容（核心修改：所有父容器改为Tabs.xgo3）
local function InitTeleportTab()
    local tab = Tabs.xgo3 -- 绑定到目标标签页
    ...
    tab:Textbox({
        ...
        Callback  = function(inputName)
            ...
            elseif #matchPlayers == 1 then
                ...
                local matchNames = {}
                for _, p in ipairs(matchPlayers) do table.insert(matchNames, p.Name) end
    ...
    tab:Button({
        ...
        Callback = function()
            ...
            for name, _ in pairs(PlayerButtons) do
                pcall(function() PlayerButtons[name].Button:Destroy() end)
                if PlayerButtons[name].Conn then PlayerButtons[name].Conn:Disconnect() end
            ...
            for _, plr in ipairs(currentOnlinePlayers) do
                coroutine.wrap(function() CreateSinglePlayerButton(plr, tab) end)()
            end
            RefreshDropdown()
    ...
    -- 方式三标题：直接点击玩家按钮
    tab:Block("=== 直接点击玩家按钮 ===", nil, nil, nil, nil, true)
    ...
    Players.PlayerAdded:Connect(function(plr)
        ...
        coroutine.wrap(function()
            CreateSinglePlayerButton(plr, tab)
            RefreshDropdown()
    ...
    Players.PlayerRemoving:Connect(function(plr)
        if plr == LocalPlayer then return end
        -- 清理按钮
        if PlayerButtons[plr.Name] then
            pcall(function() PlayerButtons[plr.Name].Button:Destroy() end)
            PlayerButtons[plr.Name].Conn:Disconnect()
...
-- 7. 启动传送标签页初始化（确保本地玩家加载完成）
local function StartTeleportUI()
    ...
    InitTeleportTab()
    SendMsg("UI加载完成", "按 LeftControl 打开/关闭UI，传送功能已就绪", "default")
end
