...
-- ===================== 传送核心变量 ===================== --
-- 2. 基础服务与全局表（优化：存储按钮+监听连接，方便彻底清理）
local Players      = game:GetService("Players")
local LocalPlayer  = Players.LocalPlayer
local MainWindow, Tabs.xgo3
local PlayerButtons = {}   -- name -> {Button, Conn}，避免引用残留
local PlayerDropdown       -- 下拉控件引用
local selectedName  = ""    -- 下拉选中的玩家名

-- 3. 小工具（含模糊匹配函数，确保匹配后立即传送逻辑）
local function SendMsg(title, desc, type)
    Notification:Notify(
        { Title = title, Description = desc },
        { OutlineColor = Color3.fromRGB(80,80,80), Time = 3, Type = type or "default" },
        { Image = "https://www.roblox.com/asset/?id=6023426923", ImageColor = Color3.fromRGB(0,255,170) }
    )
end

local function CheckPlayerValid(p)
    return p and p.Parent == Players and p.Character and p.Character:FindFirstChild("HumanoidRootPart") and p.Character.Humanoid.Health > 0
end

local function TeleportToPlayer(target)
    if not CheckPlayerValid(LocalPlayer) then
        SendMsg("传送失败","你的角色未加载或已死亡","default"); return
    end
    if CheckPlayerValid(target) then
        LocalPlayer.Character.HumanoidRootPart.CFrame = target.Character.HumanoidRootPart.CFrame
        SendMsg("传送成功","已传送到 "..target.Name,"default")
    else
        SendMsg("传送失败",target.Name.." 角色未加载或已死亡","default")
    end
end
...
-- 核心修改：获取下拉列表选项时，第一个选项固定为“选择玩家”
local function GetDropdowndefaults()
    local defaults = { "选择玩家" } -- 第一个选项默认为“选择玩家”
    local playerNames = {}
    -- 补充真实在线玩家名
    for _, p in ipairs(GetOtherPlayers()) do
        table.insert(playerNames, p.Name)
    end
    -- 合并选项（“选择玩家”+真实玩家名，无玩家时仅显示“选择玩家”）
...
-- 模糊匹配函数：输入部分名称，返回所有匹配的玩家
local function FuzzyMatchPlayers(inputStr)
    local matchList = {}
    local lowerInput = string.lower(inputStr) -- 忽略大小写匹配
    for _, p in ipairs(GetOtherPlayers()) do
        local lowerPlayerName = string.lower(p.Name)
        if string.find(lowerPlayerName, lowerInput) then
...
-- 4. 动态UI核心（确保按钮清理逻辑正常）
local function CreateSinglePlayerButton(player)
    -- 先清理已存在的按钮和监听
    ...
    -- 人物加载超时保护（5秒）
    local charLoaded = player.Character or player.CharacterAdded:Wait(5)
    if not charLoaded or not CheckPlayerValid(player) then
        SendMsg("创建失败",player.Name.." 角色加载超时或已离线","default"); return
    end

    -- 监听玩家销毁，主动清理按钮
    local playerDestroyConn = player.Destroying:Connect(function()
        RemovePlayerButton(player.Name)
        playerDestroyConn:Disconnect()
    end)

    -- 创建按钮并存储
    local btn = Tabs.xgo3:Button({
        Title   = player.Name,
        Content = "点击直接传送\n目标：" .. player.Name,
        Tip     = "点击传送到" .. player.Name,
        Callback = function()
    ...
    PlayerButtons[player.Name] = {
        Button = btn,
        Conn = playerDestroyConn
    }
end

local function RemovePlayerButton(playerName)
    if PlayerButtons[playerName] then
        pcall(function() PlayerButtons[playerName].Button:Destroy() end)
        if PlayerButtons[playerName].Conn then
            PlayerButtons[playerName].Conn:Disconnect()
        end
        PlayerButtons[playerName] = nil
        SendMsg("玩家清理", playerName .. " 的传送按钮已彻底删除", "default")
    end
end

-- 核心修改：刷新下拉列表时，保持第一个选项为“选择玩家”
local function RefreshDropdown()
    local dropdowndefaults = GetDropdowndefaults()
    if PlayerDropdown and PlayerDropdown.Values then
        PlayerDropdown.Values  = dropdowndefaults
        PlayerDropdown.Default = dropdowndefaults[1] -- 默认选中第一个选项“选择玩家”
        -- 强制更新下拉列表显示
        ...
        for _, opt in ipairs(dropdowndefaults) do
        ...
        if not isTargetValid then
            selectedName = "选择玩家"
            SendMsg("选中重置", "之前选中的玩家已离线，已重置为“选择玩家”", "default")
        end
    end
end
--  ================ 脚本ui通知 ================= --
local NotificationHolder = loadstring(game:HttpGet('https://github.com/GTAFAW/UI/raw/main/通知n1.lua'))()
local Notification = loadstring(game:HttpGet('https://github.com/GTAFAW/UI/raw/main/通知n2.lua'))()
-- 新增：定义极简通知函数 SendMsg（适配原通知库逻辑，支持option类型和按钮）
local function SendMsg(title, desc, type)
    local notifyParams = {
        { Title = title, Description = desc },
        { OutlineColor = Color3.fromRGB(80, 80, 80), Time = 3, Type = type or "default" },
    ...
    return function(buttons)
        if buttons then notifyParams[3].Buttons = buttons end
        Notification:Notify(unpack(notifyParams))
    end
end
--  ================  AcrylicLibrary.UI ================= --
...
local Window = AcrylicLibrary:Windowxgo({
    ...
    KeySystemInfo = {
        ...
        OnGetKey = function()
            ...
            SendMsg("✅ 复制成功", "QQ群号 " .. qqGroupNum .. " 已复制，默认卡密1234")("option")({
                Buttons = {
                    {
                        Text = "打开群链接",
                        Color = Color3.fromRGB(80, 200, 80),
                        Callback = function() game:GetService("GuiService"):OpenBrowserWindow(qqGroupUrl) end
                    }
                },
                ImageColor = Color3.fromRGB(80, 200, 80)
            })
            return qqGroupUrl
        end,
        OnLogin = function(key)
            ...
            if key == "1234" then
                wait(0.1)
                SendMsg("🎉 验证通过", "已成功登录，可正常使用功能")("option")
                return true -- 卡密正确
            end
            SendMsg("❌ 卡密错误", "默认卡密是1234，请重新输入")("option")
            wait()
            return false -- 卡密错误
        end,
    }
