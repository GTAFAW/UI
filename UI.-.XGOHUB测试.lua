...
function Root:CheckBox(setup)
	setup = setup or {}
	setup.Title      = setup.Title or "复选框"
	setup.Content    = setup.Content or ""
	setup.Default    = setup.Default or false
	setup.Callback   = setup.Callback or function() end

	local box = Instance.new("Frame")
	box.Name = "CheckBoxBlock"
	box.Parent = ScrollingFrame
	box.BackgroundColor3 = Library.Colors.Default
	box.BackgroundTransparency = 0.25
	box.BorderSizePixel = 0
	box.Size = UDim2.new(0.99,0,0,Library.ItemHeight)
	box.ZIndex = 10
	...
	shadow.SliceCenter = Rect.new(95,103,894,902)
	...
	title.RichText = true
	title.AnchorPoint = Vector2.new(0,0.5)
	title.Position = UDim2.new(0,24+8,0.5,0)
	title.Size = UDim2.new(1,-(24+8+5),0.4,0)
	...
	content.Position = UDim2.new(0,24+8,0,18)
	content.Size = UDim2.new(1,-(24+8+5),0,45)
	content.Visible = false
	content.Font = Enum.Font.Gotham
	content.Text = setup.Content
	content.TextColor3 = Library.Colors.TextColor
	...
	checkFrame.Parent = box
	checkFrame.AnchorPoint = Vector2.new(0,0.5)
	checkFrame.Position = UDim2.new(0,8,0.5,0)
	checkFrame.Size = UDim2.fromOffset(18,18)
	checkFrame.BackgroundTransparency = 1
	checkFrame.ZIndex = 12
	...
	boxStroke.Color = Library.Colors.TextColor
	...
	Library:MakeDrop(box, stroke, Library.Colors.Hightlight)
	if setup.Tip then WindowLibrary:AddToolTip(box, tostring(setup.Tip)) end

	--------------------------------------------------
	--  动画 & 状态
	--------------------------------------------------
	local tweenInfoPop = TweenInfo.new(0.25, Enum.EasingStyle.Back, Enum.EasingDirection.Out)
	...
	local function setState(value)
		...
		if value then
			-- 勾弹出
			Library:Tween(checkBox, {Duration=0.15}, {BackgroundColor3 = Library.Colors.Hightlight, BackgroundTransparency = 0})
			Library:Tween(boxStroke, {Duration=0.15}, {Color = Library.Colors.Hightlight, Transparency = 0})
			checkMark.TextTransparency = 0
			checkMark.Size = UDim2.fromScale(0,0)
			Library:Tween(checkMark, tweenInfoPop, {Size = UDim2.fromScale(1,1)})
		else
			Library:Tween(checkBox, {Duration=0.15}, {BackgroundColor3 = Color3.new(1,1,1), BackgroundTransparency = 1})
			Library:Tween(boxStroke, {Duration=0.15}, {Color = Library.Colors.TextColor, Transparency = 0.4})
			Library:Tween(checkMark, {Duration=0.1}, {TextTransparency = 1})
		end
	...
	ripple.Parent = checkBox
	ripple.AnchorPoint = Vector2.new(0.5,0.5)
	ripple.Size = UDim2.fromOffset(0,0)
	ripple.Position = UDim2.new(0,px-checkBox.AbsolutePosition.X,0,py-checkBox.AbsolutePosition.Y)
	...
	ripple.ZIndex = 14
		...
		Library:Tween(ripple, tweenInfoRipple, {Size = UDim2.fromOffset(40,40), BackgroundTransparency = 1})
		game:GetService("Debris"):AddItem(ripple, 0.35)
	end

	btn.MouseButton1Click:Connect(function(input)
		spawnRipple(input.Position.X, input.Position.Y)
		-- 微缩反馈
		Library:Tween(checkFrame, {Duration=0.05}, {Size = UDim2.fromOffset(16,16)})
		task.wait(0.05)
		Library:Tween(checkFrame, {Duration=0.1, EasingStyle=Enum.EasingStyle.Back}, {Size = UDim2.fromOffset(18,18)})
		setState(not setup.Default)
	end)
	...
	local function updateHeight()
		local hasContent = setup.Content:len() > 0
		content.Visible = hasContent
		if hasContent then
			title.Position = UDim2.new(0,24+8,0,8)
			title.Size = UDim2.new(1,-(24+8+5),0,14)
			local h = 8 + 14 + 5 + content.TextSize * 2
			box.Size = UDim2.new(0.99,0,0,h)
		else
			title.Position = UDim2.new(0,24+8,0.5,0)
			title.Size = UDim2.new(1,-(24+8+5),0.4,0)
			box.Size = UDim2.new(0.99,0,0,Library.ItemHeight)
	...
	--------------------------------------------------
	local api = {}
	function api:Content(str)
		setup.Content = str
		content.Text = str
		updateHeight()
	end
	function api:Value(v)
		if v == nil then return setup.Default end
		setState(v)
	end
	function api:Visible(b) box.Visible = b end

	setState(setup.Default)
	return api
end
...
local function createEmitter(parent, name, color)
	...
	pe.Lifetime = NumberSequence.new(2,3)
...
local function toColorSeq(c3)
	return ColorSequence.new{
		ColorSequenceKeypoint.new(0, c3),
...
function Particles:Init(screenGui)
	if self._root then return self end
	self._root = Instance.new("Frame")
	self._root.Name = "ParticleLayer"
	self._root.Size = UDim2.fromScale(1,1)
	self._root.BackgroundTransparency = 1
	self._root.BorderSizePixel = 0
	self._root.ZIndex = -10
	self._root.Parent = screenGui
	self._bg = createEmitter(self._root, "BG", ColorSequence.new(Color3.fromRGB(240,240,240)))
	self._fg = createEmitter(self._root, "FG", ColorSequence.new(Color3.fromRGB(200,200,200)))
	self._hl = createEmitter(self._root, "HL", toColorSeq(Library.Colors.Hightlight))
	self._themeConn = Library:GetPropertyChangedSignal("Colors"):Connect(function()
		self._hl.Color = toColorSeq(Library.Colors.Hightlight)
	end)
	self:Stop()
	return self
end

function Particles:Play()
	if not self._root then return end
	self._bg.Enabled = true
	self._fg.Enabled = true
	self._hl.Enabled = true
end

function Particles:Stop()
	if not self._root then return end
	self._bg.Enabled = false
	self._fg.Enabled = false
	self._hl.Enabled = false
end
...
local ScreenGui = Instance.new("ScreenGui")
Library.Particles:Init(ScreenGui)
...
local function Min()
	...
	if WindowLibrary.Toggle then
		...
		Library:Tween(Ico, Library.TweenLibrary.WindowChangedFast, { ImageTransparency = 0.25 })
		Library:Tween(Block, Library.TweenLibrary.WindowChangedFast, { Position = UDim2.new(0.5, 0, -1.5, 0) })
		Library:Tween(MainFrame, Library.TweenLibrary.SmallEffect, { Size = Library.SizeLibrary.Close })
		Library:Tween(MainFrame, Library.TweenLibrary.WindowChangedFast, { Position = UDim2.fromOffset(29, 27) })
		Library.Particles:Stop()
	end
end
